...
- `npm test` — runs the TypeScript tests with Node's built-in test runner and type stripping. CI runs this before the production build.
...
- Runner tests live in `tests/valeRunner.test.ts`. Keep Vale process/JSON behavior covered there; editor lifecycle behavior still requires manual Obsidian testing.
...
- **`main.ts`** — the plugin entry point (`ValePlugin extends Plugin`) and `ValeSettingTab`. Owns settings (`ValePluginSettings`, persisted via `loadData`/`saveData`), the status bar item, command registration, and orchestrates checks on `editor-change` (debounced) or `active-leaf-change`. It snapshots `Editor.getValue()`, retains the originating editor/view, rejects stale asynchronous results, and dispatches accepted `ValeIssue[]` results to the CodeMirror decoration layer.
- **`src/valeRunner.ts`** — the Vale process boundary. It sends the in-editor text over stdin and supplies the vault-relative note path with `--path`, so Vale still applies Markdown and path-specific configuration without a temporary file. It parses JSON output and treats Vale's exit code 1 ("alerts found") as a successful lint result. Keep process invocation here so it can be tested without loading Obsidian.
- **`src/valeDecorations.ts`** — the CodeMirror 6 integration: a `StateField`/`StateEffect` pair (`valeDecorationsField` / `setValeDecorationsEffect`) that turns `ValeIssue[]` into a `DecorationSet`, plus a `hoverTooltip` extension that renders severity/message/suggestion-action UI on hover. Hover tooltips must be anchored with both `pos` and `end` from the full issue range and remain pointer-interactive, otherwise they close while the user moves across the highlighted text or into the suggestion controls. Suggestion/replace/remove actions are applied by dispatching editor transactions directly against calculated `{from, to}` ranges (Vale's 1-indexed line/column spans converted to CodeMirror's 0-indexed doc positions). The shared `ValeIssue` type comes from `src/valeRunner.ts`.
